...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (dateFrom) {
      ...
      result = result.filter(item => new Date(item.checkin_time) >= from);
    }
    if (dateTo) {
      const to = new Date(dateTo);
      to.setHours(23, 59, 59, 999);
      result = result.filter(item => new Date(item.checkin_time) <= to);
    }
    if (search) {
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto">
        <h2 className="text-2xl font-bold mb-6">Attendance Data</h2>

        <div className="grid grid-cols-4 gap-4 mb-6">
          <StatCard title="Total Employee" value={stats.totalEmployee} color="blue" />
          <StatCard title="Checked-in Today" value={stats.checkedInToday} color="green" />
          <StatCard title="Absent Today" value={stats.absentToday} color="red" />
          <StatCard title="Attendance Rate" value={`${stats.attendanceRate}%`} color="purple" />
        </div>
            ...
            <tbody>
              ...
              {paginated.map(item => (
                <tr key={item.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-4">{item.employee.department || "-"}</td>
                  <td className="p-4">
...
function Modal({ item, onClose }: any) {
  return (
    ...
    >
      ...
      >
        <h3 className="font-bold mb-2">{item.employee.name}</h3>
        <img
          src={`http://localhost:3000/uploads/${item.photo}`}
